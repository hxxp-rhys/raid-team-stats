...
server, edit **one file** (`.env`), and run two commands (`./init.sh`, then
`docker compose up -d`). It pulls a pre-built image — there is nothing to compile.

The whole thing is three steps:

```bash
./init.sh                 # 1. create .env, generate secrets, prepare storage
$EDITOR .env              # 2. set APP_HOST/APP_URL/AUTH_URL + your API keys
...
Run the init script. In one command it creates `.env` from the template,
generates every secret and password, and prepares the data directories with the
ownership each container needs:

```bash
sudo ./init.sh
```

> **Why `sudo`?** It creates data directories that Prometheus, Loki, and Grafana
> (non-root) and Caddy (root) must each own; without it those services fail to
> start with "permission denied". `init.sh` is **re-run safe** — it never
> overwrites a secret you already have — so run it again any time you change
> `DATA_DIR` or add a custom certificate.

`init.sh` fills in `AUTH_SECRET`, `TOKEN_ENCRYPTION_KEY`, `METRICS_TOKEN`, the
Postgres/Redis/Grafana passwords, and more. `.env` is the **only file you edit** —
open it and set the handful of values only you know:
...
> **Custom data location:** all data lives in `./data` (next to the compose file)
> by default. To put it on a specific disk, set `DATA_DIR` to an absolute path
> (e.g. `DATA_DIR=/srv/raidstats/data`) in `.env` and re-run `sudo ./init.sh`.
...
3. Fix ownership/permissions so Caddy (which runs as **root with all Linux
   capabilities dropped**) can read them. `init.sh` does this and warns
...
> the app validates it at boot. `init.sh` already set it.
>
...
| `... is required — set it in .env` on `up` | A required value (`POSTGRES_PASSWORD`, `REDIS_PASSWORD`, `METRICS_TOKEN`) is blank. Run `./init.sh`. |
| Image pull `denied` / `unauthorized` | Package is private — `docker login ghcr.io` (see [Prerequisites](#prerequisites)). |
| Prometheus/Loki/Grafana keep restarting, "permission denied" | You didn't run `sudo ./init.sh` (or changed `DATA_DIR` without re-running it). |
...
| Caddy won't start, "cert/key not readable" | `TLS_MODE=custom` but the files named by `SSL_CERT_FILENAME` / `SSL_KEY_FILENAME` aren't in `<DATA_DIR>/certs` or aren't root-readable. Add them and run `sudo ./init.sh`. |
