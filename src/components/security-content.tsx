import { UPSTREAM } from "@/lib/site-config";

/**
 * Shared security copy + render blocks, used by BOTH the dedicated /security
 * page and the Security section of the combined /about page, so the wording
 * (which describes the software's actual safeguards) can never drift between
 * the two. Server components — no client state.
 */
export const SECURITY_SECTIONS: ReadonlyArray<{ title: string; body: string }> = [
  {
...
/** The six security points as a definition list. */
export function SecuritySections() {
  return (
    <dl className="space-y-5">
      {SECURITY_SECTIONS.map((s) => (
        <div key={s.title} className="border-border/60 border-l-2 pl-4">
...
/** The "Responsible disclosure" call-out box. */
